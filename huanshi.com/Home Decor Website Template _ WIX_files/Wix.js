 ...
 * @namespace Events
 */
var Events, custom, es5Shim, es5Sham, base64, polyFills, core, utils, Media, Utils, Styles, Theme, WindowOrigin, WindowPlacement, Base, Error, WixDataCursor, responseHandlers, Billing, Activities, Settings, Contacts, PubSub, Worker, Preview, Dashboard, Counters, Features, Wix;
Events = {
  /**
...
core = function (Events, polyFills) {
  ...
  var MessageTypes = {
    ...
    SETTINGS_RESIZE_COMPONENT: 'settingsResizeComponent',
...
Media = function (utils, core) {
  var wixstatic = utils.protocol() + '//static.wixstatic.com/';
  var wixmedia = utils.protocol() + '//media.wix.com/';
  var getImageUrl = function (relativeUrl) {
    return wixstatic + 'media/' + relativeUrl;
  };
  var getResizedImageUrl = function (relativeUrl, width, height, sharpParams) {
    ...
    // build the image url
    relativeUrl = wixstatic + 'media/' + relativeUrl;
  ...
  var getAudioUrl = function (relativeUrl, audioType) {
    ...
    if (!utils.has(AudioType, audioType)) {
      core.reportSdkError('Invalid argument - audioType value should be set using Wix.Utils.Media.AudioType');
    }
    switch (audioType) {
    case AudioType.STANDARD:
      return wixmedia + 'mp3/' + relativeUrl;
    case AudioType.PREVIEW:
      return wixstatic + 'preview/' + relativeUrl;
    case AudioType.SHORT_PREVIEW:
      return wixstatic + relativeUrl;
    }
  };
  var getDocumentUrl = function (relativeUrl) {
    return wixmedia + 'ugd/' + relativeUrl;
  };
  var getSwfUrl = function (relativeUrl) {
    return wixstatic + 'media/' + relativeUrl;
  };
  var getPreviewSecureMusicUrl = function (previewFileName) {
    core.reportSdkMsg('Wix.Utils.Media.getPreviewSecureMusicUrl is DEPRECATED please use Wix.Utils.Media.getAudioUrl(\'myFileName.mp3\', Wix.Utils.Media.AudioType.PREVIEW)');
    return wixstatic + 'preview/' + previewFileName;
  };
  var AudioType = {
    STANDARD: 'STANDARD',
    PREVIEW: 'PREVIEW',
...
Styles = function (core) {
  ...
  var setEditorParam = function (type, key, value, onSuccess, onError) {
...
Base = function (core, Utils, Styles, utils, Theme, WindowOrigin, WindowPlacement) {
  var openModal = function (url, width, height, onClose, theme) {
    if (Utils.getViewMode() === 'editor') {
      core.reportSdkError('Invalid view mode. This function cannot be called in editor mode. Supported view modes are: [preview, site]');
      return;
    }
    var args = {
      url: url,
  ...
  var navigateToComponent = function (compId, options, onFailure) {
    ...
    if (!utils.isString(compId)) {
      core.reportSdkError('Missing mandatory argument - compId - should be of type String');
      return;
    }
    var args = { compId: compId };
    if (utils.isFunction(options)) {
      ...
      if (utils.isObject(options) && options.pageId && utils.isString(options.pageId)) {
        ...
        core.reportSdkError('Invalid argument - options should be of type object, containing string representing page id');
        return;
      }
    }
    core.sendMessage(core.MessageTypes.NAVIGATE_TO_COMPONENT, args, onFailure);
  };
  ...
  var currentMember = function (onSuccess) {
    if (Utils.getViewMode() !== 'site') {
      core.reportSdkError('Invalid view mode. This function cannot be called in editor/preview mode. Supported view mode is: [site]');
      return;
    }
  ...
  var getExternalId = function (onSuccess, onError) {
    if (!onSuccess) {
      core.reportSdkError('Mandatory arguments - an onSuccess callback must be specified');
    }
    var callback = function (data) {
      if (data && data.onError) {
        if (onError) {
    ...
    core.sendMessage(core.MessageTypes.GET_EXTERNAL_ID, undefined, callback);
  };
  return {
    /**
...
responseHandlers = function (core, Error, WixDataCursor) {
  var getWixError = function (errorCode) {
    ...
    switch (errorCode) {
    case 403:
      wixErrorMessage = Error.FORBIDDEN;
      break;
    case 404:
...
Settings = function (core, Styles, Base, WindowPlacement, utils, responseHandlers) {
  var getStyleParams = function (callback) {
  ...
  var resizeComponent = function (options, onSuccess, onFailure) {
    if (!options || !options.width && !options.height) {
      core.reportSdkError('Mandatory arguments - width or height must be supplied');
      return;
    }
    var args = {};
    if (options.width) {
    ...
    core.sendMessage(core.MessageTypes.SETTINGS_RESIZE_COMPONENT, args, callback);
  };
  var addComponent = function (options, onSuccess, onError) {
    if (!options || !options.componentType) {
      core.reportSdkError('Mandatory arguments - options has to have componentType');
      return;
    }
    var callback = function (data) {
      if (data.onError) {
    ...
    var args = { componentType: options.componentType };
    if (options.componentType === 'WIDGET') {
      if (!options.widget) {
        core.reportSdkError('Mandatory arguments - options has to have widget object');
        return;
      }
      args.widget = {
    ...
    if (options.componentType === 'PAGE') {
      if (!options.page) {
        core.reportSdkError('Mandatory arguments - options has to have page object');
        return;
      }
      args.page = {
    ...
    core.sendMessage(core.MessageTypes.ADD_COMPONENT, args, callback);
  };
  var setExternalId = function (guid, onSuccess, onError) {
    if (!guid) {
      core.reportSdkError('Mandatory arguments - GUID must be provided');
      return;
    }
    var callback = function (data) {
      if (data.onError) {
    ...
    var args = { externalId: guid };
    core.sendMessage(core.MessageTypes.SET_EXTERNAL_ID, args, callback);
  };
  var revalidateSession = function (onSuccess, onError) {
    if (onSuccess) {
      if (utils.isFunction(onSuccess)) {
        ...
        core.sendMessage(core.MessageTypes.REVALIDATE_SESSION, {}, callback);
      } else {
        core.reportSdkError('Mandatory argument - onSuccess - should be of type Function');
      }
    } else {
      core.reportSdkError('Missing Mandatory argument - onSuccess');
    }
  };
  return {
    /**
     ...
     * Get a newly signed app instance by calling Wix.Utils.revalidateSession.
     * @function
     * @memberof Wix.Settings
     * @since 1.47.0
...
}(core, Styles, Base, WindowPlacement, utils, responseHandlers);
Contacts = function (core, utils, responseHandlers) {
  ...
  var reconcileContact = function (contactInfo, onSuccess, onFailure) {
    ...
    if (validation.passed) {
      ...
      core.sendMessage(core.MessageTypes.RECONCILE_CONTACT, contactInfo, onComplete);
    } else {
      core.reportSdkError(validation.error);
    }
  };
  return {
    /**
    ...
    /**
     * Send contact information for reconciliation using contact merging heuristics
     * @memberof Wix.Contacts
     * @author benk@wix.com
     * @since 1.46.0
     * @function
     * @param {Object} contactInfo The contact information that will be passed to the server
     * @param {Function} [onSuccess] A function that receives an array of labels.
     * @param {Function} [onFailure] A function called when an error occurs that receives a Wix.Error.
     * @return {String[]} An array of labels
     */
...
Preview = function (core) {
  var openSettingsDialog = function (options, failure) {
    core.sendMessage(core.MessageTypes.OPEN_SETTINGS_DIALOG, options, failure);
  };
  return {
    /**
     ...
     * @function
     * @param [Object] options
     * @param [Function] failure callback invoked if the user cancels opening this dialog. Can happen when called with bad component information.
     * @example
  ...
  var isSupported = function (featureName, callback) {
    if (featureName) {
      if (utils.isFunction(featureName)) {
        core.reportSdkError('Mandatory argument - feature name must be supplied.');
        return;
      }
      if (!callback) {
        core.reportSdkError('Mandatory argument - callback must be supplied.');
        return;
      }
      if (!utils.isFunction(callback)) {
        core.reportSdkError('Mandatory argument - callback must be a function.');
        return;
      }
      if (!isValidFeatureName(featureName)) {
        core.reportSdkError('Mandatory argument - feature must be one of Wix.Features.Types.');
        return;
      }
      var featureData = { name: featureName };
      core.sendMessage(core.MessageTypes.IS_SUPPORTED, featureData, callback);
    } else {
      core.reportSdkError('Mandatory arguments - feature name and callback must be supplied.');
    }
  };
  return {
    /**
...
Wix = function (core, Base, Billing, utils, Activities, Settings, Contacts, Utils, Styles, Events, Error, Media, WindowOrigin, WindowPlacement, Worker, PubSub, Preview, Dashboard, Theme, Counters, Features) {
...
}(core, Base, Billing, utils, Activities, Settings, Contacts, Utils, Styles, Events, Error, Media, WindowOrigin, WindowPlacement, Worker, PubSub, Preview, Dashboard, Theme, Counters, Features);
window.Wix = Wix;
}());