...
class Upload {
    ...
    var $thumb_dir = "thumb/"; 
    var $upfile_type = "jpg,gif,png"; 
    var $upfile_size_max = '2048'; 
    var $to_file = true; 
    ...
    function upload_image($upfile, $image_name = '') {
        ...
        for($i=0;$i<$img_count-1;$i++){
            $image_name .= $this->Pinyin($name[$i],1);
        }
    ...
    function make_thumb($photo, $width = '128', $height = '128', $thumb_flag = false, $quality = '100', $auto_change = true) {
        $img_info = $this->get_img_info($photo);
        $photo = $this->images_dir . $photo; 
        if($thumb_flag){
            $thumb_name = "thumb_" . substr($img_info["name"], 0, strrpos($img_info["name"], ".")) . "." . $img_info["ext"]; 
        }else{
            $thumb_name = substr($img_info["name"], 0, strrpos($img_info["name"], ".")) . "_thumb." . $img_info["ext"]; 
        }
        if ($img_info["type"] == 1) {
