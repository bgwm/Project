...
class Common extends DbMysql {
    ...
    function get_page_image_list($page_id = '',$temp_num = ''){
        $sql_page_id = $page_id?"OR page_id='".$page_id."'":"";
        $sql = "SELECT * FROM" . $this->table('page_images') . "WHERE temp_num='" . $temp_num . "'" . $sql_page_id;
        $query = $this->query($sql);
        while ($row = $this->fetch_array($query)){
            $image = explode('.', basename($row['image_url']));
            $thumb =$thumb = $GLOBALS['images_dir'] . $GLOBALS['thumb_dir'] . $image['0'] . "_thumb." . $image['1'];

            $image_list[] = array(
              "id" => $row['id'],
                "sort" => $row['sort'],
                "name" => $row['name'],
                "image_url" => ROOT_URL .$row['image_url'],
                "temp_num" => $row['temp_num'],
