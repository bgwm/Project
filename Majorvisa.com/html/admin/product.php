...
elseif ($rec == 'insert') {
    ...
    if ($_FILES['product_image']['name'] != '') {
        $upfile[0] = $img->upload_image('product_image', $id); // 上传的文件域
        $file = $images_dir . $upfile[0];
        $img->make_thumb($upfile[0], $_CFG['thumb_width'], $_CFG['thumb_height']);
    }
    for($i=1;$i<=5;$i++){//添加附属图片input名为product_image_1样式的
    ...
    $sql_new_name = "";
    if ($_FILES['product_image']['name'] != '') {//主图
        // 格式化图片名称

        $file = $images_dir . $upfile[0];
        $no_ext = explode('.', $file);
        $file_thumb = $no_ext[0] . '_thumb' . '.' . $no_ext[1];
        $new_name = $images_dir . $good_id . '_' . $upfile[0];//$no_ext. '.' . $no_ext[1];
        $new_name_thumb = $images_dir . 'thumb_' . $good_id . '_' . $upfile[0];//$no_ext . '_' . '.' . $no_ext[1];
        $sql_new_name = $new_name;
        // 图片存在的话，先删除图片
        @ unlink('../' . $new_name);
    ...
    for($i=1;$i<=5;$i++){ //附加图
        if ($_FILES['product_image_'.$i]['name'] != '') {
            ...
            $file = $images_dir . $upfile[$i];
            $no_ext = explode('.', $file);
            $file_thumb = $no_ext[0] . '_thumb' . '.' . $no_ext[1];
            $new_name = $images_dir . $good_id . '_' . $upfile[$i];//$file .  '.' . $no_ext[1];
            $new_name_thumb = $images_dir  . 'thumb_'. $good_id . '_' .$upfile[$i];// $file . '_thumb' . '.' . $no_ext[1];
            $sql_new_name .= "," . $new_name;
            // 图片存在的话，先删除图片
            @ unlink('../' . $new_name);
            @ unlink('../' . $new_name_thumb);
    ...
    $resql = "update " . $dou->table('product') . " SET product_image='$sql_new_name' WHERE id='$good_id'";
    $dou->query($resql);
...
elseif ($rec == 'edit') {
    ...
    foreach (explode(',',$product['product_image']) as $pi) {
        $rpi[$rpiIndex] = ROOT_URL.$pi;//图片绝对地址
        //生成缩略图绝对地址
       // $rtpiSplit = explode('/', $pi);
       // $rtpiFn = $rtpiSplit[count($rtpiSplit)-1];
       // $rtpiPath = str_replace($rtpiFn,'',$pi);
        $rtpi[$rpiIndex] = ROOT_URL . dirname($pi) . '/thumb_' . basename($pi);
        $rpiIndex++;
    }
    ...
    $smarty->assign('product_category', $dou->get_category_nolevel('product_category'));
    $smarty->assign('product', $product);
    $smarty->assign('product_images', $rpi);//输出商品图片数组
    $smarty->assign('product_image_thumbs', $rtpi);
...
elseif ($rec == 'update') {
    ...
    // 上传图片生成
    $oldPi = explode(',',$dou->get_one("SELECT product_image FROM " . $dou->table('product') . " WHERE id = '$_POST[id]'"));//获得原来的图片地址
    if ($_FILES['product_image']['name'] != "") {
        // 分析商品图片名称
        ...
        $img->make_thumb($upfile, $_CFG['thumb_width'], $_CFG['thumb_height'], true);
        
        $up_file = $file;
    }else{
        $up_file = $oldPi[0];//如果没有新的上传则添加原来的图片地址
    }

    for($i=1;$i<=5;$i++){
        if ($_FILES['product_image_'.$i]['name'] != "") {
            // 分析商品图片名称
            $basename = basename($_POST['product_image_'.$i]);
            $file_name = substr($basename, 0, strrpos($basename, '.'));

            $upfile = $img->upload_image('product_image_'.$i, $file_name); // 上传的文件域
            $file = $images_dir . $upfile;
            $img->make_thumb($upfile, $_CFG['thumb_width'], $_CFG['thumb_height'], true);

            $up_file .= ",".$file;
        }else{
           if($oldPi[$i] != '' || $oldPi[$i] != null){
               $up_file .= ",".$oldPi[$i];//检查原来是否有图片地址，如果没有则不要添加
           }
        }
    }
    $up_file = ", product_image='$up_file'";
    // CSRF防御令牌验证
