...
class Action extends Common {
    ...
    function get_catalog($module = '', $id = '') {
        ...
        // 招聘分类列表
        $job_category = $this->get_category_nolevel('job_category');
        $job_category_array[] = array (
            "name" => $GLOBALS['_LANG']['nav_job'],
            "module" => 'job_category',
            "cur" => ($module == 'job_category' && $id == 0) ? true : false,
            "guide" => 0
        );
        foreach ($job_category as $row) {
            $job_category_array[] = array (
                ...
                "module" => 'job_category',
                "guide" => $row['cat_id'],
                "cur" => ($module == 'job_category' && $id == $row['cat_id']) ? true : false,
                "mark" => '-' . $row['mark']
            );
        }

        $catalog = array_merge($catalog, $job_category_array);
    ...
    function dou_api() {
        ...
        $apiget = "ver=$_CFG[douphp_version]&lang=$_CFG[language]&php_ver=$sys_info[php_ver]&mysql_ver=$sys_info[mysql_ver]&os=$sys_info[os]&web_server=$sys_info[web_server]&charset=$sys_info[charset]&template=$_CFG[site_theme]&url=" .
                 ROOT_URL;
        return "http://api.1ze.cn/update.php" . '?' . $apiget;
    }
     ...
     * +----------------------------------------------------------
     * $file 图片文件
     * $sys_width 用户设置的默认缩略图宽
     * $sys_height 用户设置的默认缩略高
     * +----------------------------------------------------------
     */
    function get_image_size($file, $sys_width, $sys_height){
        $img_info = GetImageSize($file);
        $img_width = $img_info['width'];
        $img_height = $img_info['height'];
        $img_size = array("width"=>"0","height"=>"0");
        //自动缩放，宽度优先算法
        $img_size['width'] = $sys_width;
        $img_size['height'] = $sys_height;
        if ($img_width > $img_height){
            if ($sys_width < $sys_height){
                $img_size['height'] = floor($sys_width * ($sys_width / $sys_height));
            }
        }else{
            if ($sys_width > $sys_height){
                $img_size['height'] = floor($sys_width * ($sys_width / $sys_height));
